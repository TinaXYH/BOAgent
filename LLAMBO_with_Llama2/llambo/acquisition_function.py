...
Hyperparameter configuration:"""
    ...
    async def _async_generate(self, user_message):
        ...
        for retry in range(MAX_RETRIES):
            ...
            self.rate_limiter.add_request(request_token_count=len(resp['choices'][0]['text'].split()), current_time=start_time)
            # print('output: ')
            # print(resp)
            break
        ...
        tot_tokens = len(resp['choices'][0]['text'].split())
        tot_cost = 0
        return resp, tot_cost, tot_tokens
        ...
        while number_candidate_points < 5:
            ...
            for response in llm_responses:
                ...
                for response_message in response[0]['choices']:
                        ...
                        response_content = response_message['text']
                        try:
