...
class LLM_DIS_SM:
    def __init__(self, task_context, n_gens, lower_is_better, 
                 ...
                 prompt_setting=None, shuffle_features=False, model_llm=None):
    ...
    async def _async_generate(self, few_shot_template, query_example, query_idx):
        ...
        for retry in range(MAX_RETRIES):
            try:
                ...
                token_count = self._count_tokens(resp)
                self.rate_limiter.add_request(request_token_count=token_count, current_time=time.time())
                break
        ...
        tot_tokens = self._count_tokens(resp)
        tot_cost = 0

        return query_idx, resp, tot_cost, tot_tokens

    def _count_tokens(self, response):
        return len(response['choices'][0]['text'].split()) if response and 'choices' in response else 0
        ...
        for i in range(0, len(query_examples), 5):
            ...
            for _, sample_response in enumerate(chunk_results):
                if not sample_response:     # if sample prediction is an empty list :(
                    ...
                    all_gens_text = [x['text'] for template_response in sample_response for x in template_response[0]['choices'] ]        # fuarr this is some high level programming
